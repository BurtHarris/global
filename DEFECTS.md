...
|---|---|---|---|---|
| **[DEF-01](#def-01-whatif-safety-bypass-with--force)** | **P0 - Critical** | `Bootstrap\Invoke-RebootAndResume.ps1` | `-WhatIf` bypassed when `-Force` is supplied | Unintended system reboot |
| **[DEF-02](#def-02-nested-runspacepool-exhaustion--deadlock)** | **P0 - Critical** | `Pool\pwsh\Server.ps1` | Nested runspace acquisition causes pool deadlock | Server freeze under concurrent load |
| **[DEF-03](#def-03-registry-mutation-before-confirmation--whatif)** | **P0 - Critical** | `Bootstrap\Invoke-RebootAndResume.ps1` | Registry written before `Read-Host` / `ShouldProcess` | Persistent RunOnce key left on abort |
| **[DEF-04](#def-04-script--path-injection-via-seedlocation)** | **P1 - High** | `Pool\pwsh\Server.ps1` | `$SeedLocation` string interpolated into `AddScript` | Parse error on quotes / code injection |
| **[DEF-05](#def-05-named-pipe-lacks-currentuseronly-security)** | **P1 - High** | `Pool\pwsh` | Named pipe allows local cross-user access | Potential unauthorized command execution |
| **[DEF-06](#def-06-premature-idle-shutdown-during-active-jobs)** | **P1 - High** | `Pool\pwsh\Server.ps1` | Server stops after 30 min while tasks are executing | Active workloads abruptly aborted |
| **[DEF-07](#def-07-unconditional-set-location-hijacks-working-dir)** | **P1 - High** | `Bootstrap\Profile.ps1` | `Set-Location 'D:\'` overrides starting directory | Breaks VS Code / Terminal integration |
| **[DEF-08](#def-08-unapproved-verb-warning-on-shell-startup)** | **P2 - Medium** | `DevTools` / `Bootstrap\Profile.ps1` | `Ensure-Docker` uses unapproved verb `Ensure` | Console warning on every shell launch |
| **[DEF-09](#def-09-powershell-51-vs-7-profile-gap-during-bootstrap)** | **P2 - Medium** | `Bootstrap\Setup.ps1` | Bootstrapping under 5.1 leaves pwsh 7 profile unconfigured | pwsh 7 lacks profile after bootstrap |
| **[DEF-10](#def-10-missing-process-path-refresh-after-tool-install)** | **P2 - Medium** | `Bootstrap\Setup.ps1` | Current session `$env:PATH` not updated after winget | `mise install` command-not-found failure |
| **[DEF-11](#def-11-unhandled-abandonedmutexexception)** | **P2 - Medium** | `Pool\pwsh\RunspacePool.psm1` | `WaitOne()` crashes on abandoned mutex | Server startup blocked after crash |
...
| **[DEF-14](#def-14-hardcoded-drive-path-in-sandbox-config)** | **P3 - Low** | `Bootstrap\TestInSandbox.wsb` | Hardcoded `D:\global` host mount | Portability failure if cloned elsewhere |
| **[DEF-15](#def-15-loss-of-typed-objects-and-streams)** | **P3 - Low** | `Pool\pwsh` | Output stringified with `Out-String`; streams discarded | Caller cannot receive objects/warnings |
...
- **Component:** `Bootstrap\Invoke-RebootAndResume.ps1` (line 77)
...
- **Component:** `Pool\pwsh\Server.ps1` (lines 129-144, 219-223)
- **Priority:** **P0 - Critical**
- **Description:**
...
- **Component:** `Bootstrap\Invoke-RebootAndResume.ps1` (lines 60-63)
- **Priority:** **P0 - Critical**
- **Description:**
...
- **Component:** `Pool\pwsh\Server.ps1` (line 134)
- **Priority:** **P1 - High**
- **Description:**
  ```powershell
...
- **Component:** `Pool\pwsh\Server.ps1` (lines 191-196) and `Pool\pwsh\RunspacePool.psm1` (lines 52, 121)
- **Priority:** **P1 - High**
- **Description:**
...
- **Component:** `Pool\pwsh\Server.ps1` (lines 204-207)
- **Priority:** **P1 - High**
- **Description:**
...
- **Component:** `Bootstrap\Profile.ps1` (line 13)
- **Priority:** **P1 - High**
- **Description:**
...
- **Component:** `DevTools\DevTools.psd1` (line 72) & `Bootstrap\Profile.ps1` (line 23)
- **Priority:** **P2 - Medium**
- **Description:**
...
- **Fix:**
  Add `-DisableNameChecking` to `Import-Module` in `Bootstrap\Profile.ps1`:
  ```powershell
...
- **Component:** `Pool\pwsh\RunspacePool.psm1` (line 77)
- **Priority:** **P2 - Medium**
- **Description:**
...
- **Component:** `Bootstrap\TestInSandbox.wsb` (line 17)
- **Priority:** **P3 - Low**
- **Description:**
...
- **Fix:**
  Document this requirement or generate `Bootstrap\TestInSandbox.wsb` dynamically during `Bootstrap\Setup.ps1`.

---

### DEF-15: Loss of Typed Objects and Streams
- **Component:** `Pool\pwsh\Server.ps1` (lines 147-156) & `Pool\pwsh\RunspacePool.psm1` (lines 130-140)
- **Priority:** **P3 - Low**
- **Description:**
